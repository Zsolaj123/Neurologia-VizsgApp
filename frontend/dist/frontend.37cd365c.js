 ...
 */ (function() {
    ...
    class SidebarManagerOptimized {
        ...
        validateElements() {
            return !!(this.elements.leftSidebar && this.elements.rightSidebar && this.elements.leftToggle && this.elements.rightToggle && this.elements.container);
        }
        loadState() {
            // Load from localStorage or check current DOM state
            this.state.leftCollapsed = this.elements.leftSidebar.classList.contains('collapsed') || this.elements.leftSidebar.classList.contains('hidden');
            this.state.rightCollapsed = this.elements.rightSidebar.classList.contains('collapsed') || this.elements.rightSidebar.classList.contains('hidden');
            this.updateUI();
        }
        setupEventListeners() {
            ...
            document.addEventListener('click', (e)=>{
                ...
                if (e.target === this.elements.leftToggle || e.target.closest('#left-sidebar-toggle')) {
                    e.preventDefault();
                    this.handleToggle('left');
                } else if (e.target === this.elements.rightToggle || e.target.closest('#right-sidebar-toggle')) {
                    e.preventDefault();
        ...
        setupObserver() {
            ...
            const observer = new MutationObserver((mutations)=>{
                mutations.forEach((mutation)=>{
                    if (mutation.type === 'attributes' && mutation.attributeName === 'class') this.loadState();
                });
            });
            const config = {
                attributes: true,
        ...
        handleToggle(side) {
            ...
            // Perform toggle
            if (side === 'left') this.toggleLeft();
            else this.toggleRight();
        ...
        animateSidebar(side) {
            ...
            const isCollapsed = side === 'left' ? this.state.leftCollapsed : this.state.rightCollapsed;
            // Add animation class
            sidebar.classList.add('animating');
            // Trigger laser effect
            this.triggerLaserEffect(sidebar);
            // Toggle state
            if (isCollapsed) this.expandSidebar(side);
            else this.collapseSidebar(side);
        ...
        triggerLaserEffect(sidebar) {
            ...
            shineEffect.offsetWidth; // Force reflow
        ...
        expandSidebar(side) {
            const sidebar = side === 'left' ? this.elements.leftSidebar : this.elements.rightSidebar;
            // Update state
            if (side === 'left') this.state.leftCollapsed = false;
            else this.state.rightCollapsed = false;
        ...
        collapseSidebar(side) {
            const sidebar = side === 'left' ? this.elements.leftSidebar : this.elements.rightSidebar;
            // Update state
            if (side === 'left') this.state.leftCollapsed = true;
            else this.state.rightCollapsed = true;
            // Add collapsed class
        ...
        updateButtonIcon(side) {
            const button = side === 'left' ? this.elements.leftToggle : this.elements.rightToggle;
            const isCollapsed = side === 'left' ? this.state.leftCollapsed : this.state.rightCollapsed;
            if (!button) return;
            // Update icon based on state
            if (side === 'left') {
                button.textContent = isCollapsed ? "\u25B6" : "\u25C0";
                button.title = isCollapsed ? "T\xe9telek megjelen\xedt\xe9se" : "\xd6sszecsuk\xe1s";
            } else {
                button.textContent = isCollapsed ? "\u25C0" : "\u25B6";
                button.title = isCollapsed ? "Tartalomjegyz\xe9k megjelen\xedt\xe9se" : "\xd6sszecsuk\xe1s";
            }
        }
        updateBodyClasses() {
            ...
            } else body.classList.add('left-sidebar-visible');
            ...
            } else body.classList.add('right-sidebar-visible');
            if (this.state.leftCollapsed && this.state.rightCollapsed) body.classList.add('both-sidebars-hidden', 'both-sidebars-collapsed');
        }
        saveState() {
    ...
    else new SidebarManagerOptimized().init();
})();

//# sourceMappingURL=frontend.37cd365c.js.map
