 ...
 */ class TocGenerator {
     ...
     */ initialize() {
        ...
        if (!this.tocContainer) throw new InitializationError('TOC container not found');
     ...
     */ generateTOC(topic) {
        ...
        window.dispatchEvent(new CustomEvent('tocGenerated', {
            detail: {
                topicId: topic.id,
                itemCount: topic.tableOfContents.length
            }
        }));
    }
    /**
     ...
     */ renderTOC(html) {
        ...
        this.tocContainer.innerHTML = `
            <div class="toc-header">
                <button class="toc-toggle" id="toc-expand-all">\xd6sszes kinyit\xe1sa</button>
            </div>
            <div class="toc-content">
                ${html || '<p class="toc-empty">Nincs tartalomjegyz\xe9k</p>'}
            </div>
        `;
        // Set up expand/collapse button
        const expandButton = document.getElementById('toc-expand-all');
        if (expandButton) expandButton.addEventListener('click', ()=>this.toggleAllItems());
    }
    /**
     * Clear TOC
     */ clearTOC() {
        if (this.tocContainer) this.tocContainer.innerHTML = '';
        this.currentTopic = null;
        this.activeItemId = null;
    }
    /**
     ...
     */ toggleAllItems() {
        ...
        if (hasCollapsed) {
            ...
            allChildren.forEach((child)=>{
                ...
                if (parentItem && parentItem.classList.contains('toc-item')) parentItem.classList.add('expanded');
            });
            allArrows.forEach((arrow)=>arrow.textContent = "\u25BC");
            if (expandButton) expandButton.textContent = "\xd6sszes becsuk\xe1sa";
        } else {
            // Collapse all
            allChildren.forEach((child)=>{
                child.classList.remove('expanded');
                const parentItem = child.previousElementSibling;
                if (parentItem && parentItem.classList.contains('toc-item')) parentItem.classList.remove('expanded');
            });
            allArrows.forEach((arrow)=>arrow.textContent = "\u25B6");
     ...
     */ ensureItemVisible(item) {
        ...
        if (itemRect.top < containerRect.top || itemRect.bottom > containerRect.bottom) item.scrollIntoView({
            behavior: 'smooth',
            block: 'nearest'
        });
    }
     ...
     */ updateActiveFromScroll() {
        ...
        if (scrollTop + clientHeight >= scrollHeight - 10) // Highlight the last header
        activeId = headers[headers.length - 1].id;
        else // Find the header that's currently in view
        for (const header of headers){
            const rect = header.getBoundingClientRect();
            const contentRect = contentArea.getBoundingClientRect();
            const relativeTop = rect.top - contentRect.top;
            if (relativeTop <= 50) activeId = header.id;
            else if (relativeTop > 50) break;
        }
        if (activeId && activeId !== this.activeItemId) this.setActiveItem(activeId);
    }
    /**
     ...
     */ getStats() {
        ...
        const countItems = (items, depth = 1)=>{
            for (const item of items){
                ...
                if (item.children && item.children.length > 0) countItems(item.children, depth + 1);
            }
        };
     ...
     */ destroy() {
        if (this.tocContainer) this.tocContainer.removeEventListener('click', this.handleTocClick);
        this.clearTOC();
    }
}
...
//# sourceMappingURL=frontend.812cf45d.js.map
