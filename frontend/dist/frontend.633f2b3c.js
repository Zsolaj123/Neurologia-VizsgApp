 ...
 */ class MarkdownParser {
     ...
     */ parseMarkdown(markdown) {
        if (!markdown || typeof markdown !== 'string') throw new ParseError('Invalid markdown input');
        try {
            // Extract sections first
            const sections = this.extractSections(markdown);
            // Parse each section separately
            const parsedSections = {};
            for (const [key, content] of Object.entries(sections))parsedSections[key] = this.convertToHTML(content);
     ...
     */ extractSections(markdown) {
        ...
        for (const line of lines){
            ...
            for (const [type, pattern] of Object.entries(this.sectionMarkers))if (pattern.test(line)) {
            ...
            if (!sectionFound) sectionContent.push(line);
        }
        // Save last section
        if (sectionContent.length > 0) sections[currentSection] = sectionContent.join('\n').trim();
        // If no sections were found, put everything in részletes
        if (!sections.reszletes && !sections.osszefoglalas && !sections.kepek) sections.reszletes = markdown;
        return sections;
    }
    /**
     ...
     */ convertLists(html) {
        ...
        if (inList) result.push(this.createList(listType, listItems));
        return result.join('\n');
    }
    /**
     * Create HTML list
     ...
     */ convertTables(html) {
        ...
        if (inTable && headerRow) result.push(this.createTable(headerRow, tableRows));
        return result.join('\n');
    }
    /**
     * Create HTML table
     ...
     */ createTable(headerRow, bodyRows) {
        const parseRow = (row)=>{
            return row.split('|').slice(1, -1) // Remove empty first and last elements
            .map((cell)=>cell.trim());
        };
...
${bodyHtml}
     ...
     */ generateId(text) {
        ...
        return text.toLowerCase().replace(/[áàäâ]/g, 'a').replace(/[éèëê]/g, 'e').replace(/[íìïî]/g, 'i').replace(/[óòöôő]/g, 'o').replace(/[úùüûű]/g, 'u').replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-').replace(/-+/g, '-').trim();
    }
    /**
     * Convert paragraphs
     * @private
     */ convertParagraphs(html) {
        ...
        return blocks.map((block)=>{
            ...
            if (block.match(/^<(h[1-6]|p|ul|ol|blockquote|pre|table|hr)/)) return block;
            return `<p>${block}</p>`;
        }).join('\n\n');
    }
    /**
     ...
     */ buildTOCTree(flatToc) {
        ...
        for (const item of flatToc){
            ...
            if (stack.length === 0) tree.push(tocItem);
            else stack[stack.length - 1].addChild(tocItem);
            stack.push(tocItem);
        }
        return tree;
    }
     ...
     */ parseSections(markdown) {
        const sections = this.extractSections(markdown);
        const result = {};
        for (const [type, content] of Object.entries(sections))if (content) result[type] = Section.fromMarkdown(type, this.getSectionTitle(type), content, this.convertToHTML(content));
        else result[type] = Section.empty(type, this.getSectionTitle(type));
        return result;
    }
    /**
     * Get section title
...
//# sourceMappingURL=frontend.633f2b3c.js.map
