 ...
 */ class UIManager {
     ...
     */ initialize() {
        ...
        for (const elem of required){
            if (!this.elements[elem]) throw new InitializationError(`Required element ${elem} not found`);
        }
     ...
     */ setupEventListeners() {
        ...
        if (this.elements.contentDisplay) this.elements.contentDisplay.addEventListener('scroll', this.debounce(()=>this.handleContentScroll(), 50));
        ...
        // Window resize
        window.addEventListener('resize', this.debounce(()=>this.adjustLayout(), 200));
        // Custom events
     ...
     */ populateTopicMenu() {
        ...
        const categories = [
            ...
            {
                id: 'clinical',
                title: "Vizsg\xe1l\xf3m\xf3dszerek, \xc1ltal\xe1nos Klinikum",
                ranges: [
                    '60-79',
                    '80-99',
                    '100-119',
                    ...
                    html += `
                        <div class="topic-range ${rangeCollapsed ? 'collapsed' : ''}" data-range="${range}">
                            <div class="range-header" onclick="uiManager.toggleRange('${range}')">
                                ...
                                <span class="range-title">${range}. t\xe9telek</span>
        ...
        html = `
            <div class="topic-menu-controls">
                <button class="control-btn" id="topic-expand-all" onclick="uiManager.toggleAllTopics()">\xd6sszes kinyit\xe1sa</button>
            </div>
        ` + html;
     ...
     */ toggleAllTopics() {
        ...
        const hasCollapsed = Array.from(allCategories).some((el)=>el.classList.contains('collapsed')) || Array.from(allRanges).some((el)=>el.classList.contains('collapsed'));
        if (hasCollapsed) {
            // Expand all
            this.expandAll();
            if (expandButton) expandButton.textContent = "\xd6sszes bez\xe1r\xe1sa";
        } else {
            // Collapse all
            this.collapseAll();
            if (expandButton) expandButton.textContent = "\xd6sszes kinyit\xe1sa";
        }
    }
    /**
     ...
     * Handle topic click
     * @private
     */ async handleTopicClick(event1) {
        const topicItem = event1.target.closest('.topic-item');
        if (!topicItem) return;
        const topicId = parseInt(topicItem.dataset.topicId);
        if (isNaN(topicId)) return;
        // Don't reload if already current
        if (this.currentTopic && this.currentTopic.id === topicId) return;
     ...
     */ async loadTopic(topicId) {
        try {
            ...
            if (this.elements.contentDisplay) this.elements.contentDisplay.scrollTop = 0;
            // Save UI state
     ...
     */ displayTopic(topic) {
        ...
        if (!section || section.isEmpty) this.displayEmptySection(this.currentSection);
        else this.elements.contentDisplay.innerHTML = this.wrapContent(section.content);
        // Update section tabs
     ...
     */ displayEmptySection(sectionType) {
        const messages = {
            'reszletes': "Nincs el\xe9rhet\u0151 r\xe9szletes le\xedr\xe1s ehhez a t\xe9telhez.",
            'osszefoglalas': "Nincs el\xe9rhet\u0151 \xf6sszefoglal\xe1s ehhez a t\xe9telhez.",
            'kepek': "Nincsenek el\xe9rhet\u0151 k\xe9pek vagy diagrammok ehhez a t\xe9telhez."
        };
        this.elements.contentDisplay.innerHTML = `
            <div class="empty-section">
                <p>${messages[sectionType] || "Ez a szakasz \xfcres."}</p>
            </div>
        `;
    }
    /**
     ...
     */ updateSectionTabs(topic) {
        const tabs = [
            ...
            {
                id: 'osszefoglalas',
                label: "\xd6sszefoglal\xe1s",
                available: topic.hasSection('osszefoglalas')
            },
            {
                id: 'kepek',
                label: "K\xe9pek",
                available: topic.hasSection('kepek')
            }
        ];
        let html = '';
        for (const tab of tabs){
            ...
            html += `
                <button class="section-tab ${isActive ? 'active' : ''} ${isDisabled ? 'disabled' : ''}"
                        ...
                        ${isDisabled ? 'disabled' : ''}>
                    ${tab.label}
                    ${isDisabled ? '<span class="unavailable">(nem el\xe9rhet\u0151)</span>' : ''}
                </button>
            `;
        }
     ...
     * @private
     */ handleSectionClick(event1) {
        const tab = event1.target.closest('.section-tab:not(.disabled)');
        if (!tab) return;
        const section = tab.dataset.section;
        if (section && section !== this.currentSection) this.switchSection(section);
    }
    /**
     ...
     */ showError(message) {
        ...
        this.elements.errorContainer.innerHTML = `
            <div class="error-message">
                <span class="error-icon">\u{26A0}\u{FE0F}</span>
                <span class="error-text">${message}</span>
                <button class="error-close" onclick="uiManager.hideError()">\xd7</button>
            </div>
        `;
     ...
     * @private
     */ handleKeyboardShortcuts(event1) {
        // Don't handle if in input field
        if (event1.target.tagName === 'INPUT' || event1.target.tagName === 'TEXTAREA') return;
        switch(event1.key){
            case '1':
                if (event1.ctrlKey || event1.metaKey) {
            ...
            case 'ArrowRight':
                ...
                break;
            case 'r':
                if (!event1.ctrlKey && !event1.metaKey && !event1.altKey) this.switchSection('reszletes');
                break;
            case 'o':
                if (!event1.ctrlKey && !event1.metaKey && !event1.altKey) this.switchSection('osszefoglalas');
                break;
            case 'k':
                if (!event1.ctrlKey && !event1.metaKey && !event1.altKey) this.switchSection('kepek');
                break;
        }
    }
    /**
     ...
     */ toggleSidebar(side) {
        ...
        if (window.sidebarManagerOptimized && window.sidebarManagerOptimized.handleToggle) window.sidebarManagerOptimized.handleToggle(side);
        else {
     ...
     */ navigateTopic(direction) {
        ...
        // Check bounds
        if (newId >= 1 && newId <= 259) this.loadTopic(newId);
    }
    /**
     ...
     */ loadUIState() {
        ...
        // Apply loaded state
        if (!appState.ui.leftSidebarVisible) this.toggleSidebar('left');
        if (!appState.ui.rightSidebarVisible) this.toggleSidebar('right');
        // Load last topic if available
        if (appState.ui.currentTopicId) setTimeout(()=>{
...
//# sourceMappingURL=frontend.1d3499d7.js.map
